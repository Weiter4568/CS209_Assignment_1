...
public class OnlineCoursesAnalyzer {
  ...
  public Map<String, Integer> getPtcpCountByInstAndSubject() {
    ...
    for (Course course : courses) {
      ...
      ptcpCountByInstAndSubject.put(
          str, ptcpCountByInstAndSubject.getOrDefault(str, 0) + participants);
    }
    List<Map.Entry<String, Integer>> entries =
        new ArrayList<>(ptcpCountByInstAndSubject.entrySet());
    ...
    switch (by) {
      case "hours":
        ...
        for (int i = 1; i <= topK; i++) {
          ...
          //                    System.out.printf(courseList.get(i - 1) + " " + list.get(i -
          // 1).getValue() + "\n");
        }
        return courseList;
      case "participants":
        ...
        for (Course course : courses) {
          if (participant.get(course.title) == null
              || participant.get(course.title) < course.participants)
        ...
        for (int i = 1; i <= topK; i++) {
          ...
          //                    System.out.printf(courseList.get(i - 1) + " " + list1.get(i -
          // 1).getValue() + "\n");
        }
        return courseList;
    }
  ...
  public List<String> searchCourses(
    ...
    return courses.stream()
        .filter(
            course ->
                Arrays.stream(
                        course.subject.replace(" ", "").split(',' + "|" + Pattern.quote("and")))
                    .anyMatch(
                        s ->
                            s.toLowerCase().contains(courseSubject.replace(" ", "").toLowerCase())))
  ...
  public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
    Map<String, List<Double>> courseCharacteristics =
        courses.stream()
            .collect(
    ...
    List<String> recommendCourse =
        courses.stream().collect(Collectors.groupingBy(Course::getNumber)).entrySet().stream()
            .map(
                stringListEntry -> {
                  Course course =
                  ...
                  List<Double> courseInfo =
                      courseCharacteristics.getOrDefault(
                          course.getNumber(), Collections.emptyList());
                  Double similarity =
                      Math.pow(age - courseInfo.get(0), 2)
            ...
            .sorted(
                Comparator.comparing((Map.Entry<String, Double> e) -> e.getValue())
                    .thenComparing((Map.Entry<String, Double> e) -> e.getKey()))
